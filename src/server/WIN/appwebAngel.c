/**
 *  appwebAngel.c - Windows Angel program for Appweb
 *
 *  The appweb angel program can run in 2 modes. In the first mode, it starts the appweb service and 
 *  restarts appweb should it fail. In the second mode, it is a simple command line program that interacts
 *  with another instance of the angel to start or stop it via the --start, --stop, --install and 
 *  --uninstall switches.
...
static void WINAPI serviceMain(ulong argc, char **argv)
{
    ...
    threadHandle = CreateThread(0, 0, (LPTHREAD_START_ROUTINE) serviceThread, 
        (void*) 0, 0, (ulong*) &threadId);
...
static void angel()
{
    ...
    MprTime             mark;
    ulong               status;
    char                *homeDir, *serviceArgs;
    char                key[MPR_MAX_FNAME], path[MPR_MAX_FNAME];
    char                dir[MPR_MAX_FNAME], cmd[MPR_MAX_FNAME];
...
{
    SC_HANDLE   svc, mgr;
    char        cmd[MPR_MAX_FNAME], key[MPR_MAX_FNAME], dir[MPR_MAX_FNAME];
    int         serviceType;
    ...
    if (svc == NULL) {
        ...
        svc = CreateService(mgr, serviceName, serviceTitle, 
    ...
    if (homeDir == 0) {
        ...
        if (getcwd(dir, sizeof(dir) - 1) == NULL) {
            mprError(mpr, "Can't get the current working directory");
            return MPR_ERR_CANT_READ;
        }
        homeDir = dir;
    }
...
static void initService()
{
    ...
    mprGetAppDir(mpr, path, sizeof(path));
    mprAllocSprintf(mpr, &serviceProgram, -1, "%s/%s.exe", path, BLD_PRODUCT);
}
...
static int initWindow()
{
    ...
    appHwnd = CreateWindow(appName, appTitle, WS_OVERLAPPED, CW_USEDEFAULT, 
        0, 0, 0, NULL, NULL, appInst, NULL);
...
static void writeToOsLog(cchar *message)
{
    ...
    char        logName[MPR_MAX_STRING], *lines[9];
    char        *cp, *value;
    int         type;
    static int  once = 0;
    ...
    mprSprintf(msg, MPR_MAX_STRING, "%s: ERROR: ", mprGetAppName(mpr));
    ...
    if (once == 0) {
        ...
        mprSprintf(logName, sizeof(logName), "HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\"
            "Services\\EventLog\\Application\\%s", appName);
        value = "%SystemRoot%\\System32\\netmsg.dll";

        if (mprWriteRegistry(mpr, logName, "EventMessageFile", value) < 0) {
            return;
        }
    }

    event = RegisterEventSource(0, appName);
    if (event) {
        /*  
