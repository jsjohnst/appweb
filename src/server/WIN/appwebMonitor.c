...
static long msgProc(HWND hwnd, uint msg, uint wp, long lp)
{
    ...
    case WM_COMMAND:
        ...
        case MA_MENU_EXIT:
             ...
             *  FUTURE
             *
             *  h = CreateDialog(appInst, MAKEINTRESOURCE(IDD_EXIT), appHwnd, 
             *      dialogProc);
             *  ShowWindow(h, SW_SHOW);
             */
#if UNUSED
            if (MessageBoxEx(NULL, "Are you sure you want to exit", mprGetAppTitle(mpr), MB_YESNO, 0) == IDYES) {
                PostMessage(hwnd, WM_QUIT, 0, 0L);
            }
#else
            PostMessage(hwnd, WM_QUIT, 0, 0L);
...
#if UNUSED
    /*
     *  Can't get window if service is set to not interact with desktop
     */
    HWND    hwnd;

    /*
     *  Search for the real server: "appweb", "Embedthis Appweb"
     */
    hwnd = FindWindow(BLD_PRODUCT, BLD_NAME);
    if (hwnd) {
        return SendMessage(hwnd, APPWEB_QUERY_PORT_MESSAGE, 0, 0L);
    }
#else

    char    path[MPR_MAX_FNAME], appPath[MPR_MAX_FNAME], dir[MPR_MAX_FNAME], portBuf[32];
    int     fd;

    mprGetAppPath(mpr, appPath, sizeof(appPath));
    mprGetDirName(dir, sizeof(dir), appPath);
