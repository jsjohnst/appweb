...
#			cd $BLD_SRC_PREFIX
#			make clean >/dev/null 2>&1 || true
#			removeIntermediateFiles '*.o' '*.lo' '*.so' '*.a' make.rules .config.h.sav make.log .changes access.log \
#				error.log '*.log.old' .dummy $BLD_PRODUCT.conf make.log
#			cd $cdir
#		fi
#	fi
}
